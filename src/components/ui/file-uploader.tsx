	...
	const onDrop = React.useCallback(
		(acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
			...
			if (onUpload && updatedFiles.length > 0 && updatedFiles.length <= maxFileCount) {
				...
				void toast.promise(onUpload(updatedFiles), {
		...
		<div className="relative flex flex-col gap-6 overflow-hidden">
			{!isDisabled && (
				<Dropzone
					...
					disabled={isDisabled}>
					{({ getRootProps, getInputProps, isDragActive }): React.JSX.Element => (
						<div
							{...getRootProps()}
							className={cn(
								"group relative grid h-52 w-full cursor-pointer place-items-center rounded-lg px-5 py-2.5 text-center transition",
                      ...
                      files (up to ${formatBytes(maxSize)} each)`
												: ` ONE file of up to ${formatBytes(maxSize)}`}
										</p>
									</div>
								</div>
							)}
