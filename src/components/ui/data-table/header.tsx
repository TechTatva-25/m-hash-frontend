...
export function DataTableColumnHeader<TData, TValue>({
	...
	const getSortIcon = () => {
		if (column.getIsSorted() === "desc") {
			return <ArrowDownIcon className="ml-2 h-4 w-4" style={{ color: isDark ? 'rgba(139, 92, 246, 0.9)' : 'rgba(139, 92, 246, 0.8)' }} />;
		} else if (column.getIsSorted() === "asc") {
			return <ArrowUpIcon className="ml-2 h-4 w-4" style={{ color: isDark ? 'rgba(139, 92, 246, 0.9)' : 'rgba(139, 92, 246, 0.8)' }} />;
		} else {
			return <TiArrowUnsorted className="ml-2 h-4 w-4" style={{ color: isDark ? 'rgba(203, 213, 225, 0.8)' : 'rgba(100, 116, 139, 0.8)' }} />;
		}
	};
		...
		<div className={cn("flex items-center justify-center space-x-2 text-center", className)}>
			<DropdownMenu>
				...
				>
					...
					>
						<ArrowUpIcon className="mr-2 h-3.5 w-3.5" style={{ 
					...
					>
						<ArrowDownIcon className="mr-2 h-3.5 w-3.5" style={{ 
							color: isDark ? 'rgba(139, 92, 246, 0.8)' : 'rgba(139, 92, 246, 0.6)'
						}} />
						Desc
					</DropdownMenuItem>
					...
					>
						<EyeOffIcon className="mr-2 h-3.5 w-3.5" style={{ 
