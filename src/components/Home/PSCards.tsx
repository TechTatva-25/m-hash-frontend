...
export function PSCards({ filter, problems }: PSCardsProps): React.JSX.Element {
	...
	return (
		<div className="relative py-8">
			...
			<div className="absolute -bottom-20 -right-20 w-96 h-96 bg-purple-400/10 dark:bg-purple-700/10 rounded-full blur-3xl pointer-events-none animate-pulse-slow" style={{ animationDelay: "1s" }}></div>
			...
			<Carousel
				...
				<CarouselContent className="flex py-4 px-20">
					{(filter === "all" ? problems : problems.filter((problem) => problem.type === filter)).map(
						(ps, index) => (
							<CarouselItem key={index} className="group flex-shrink-0 md:basis-1/2 lg:basis-1/3 px-3 py-2">
								<motion.div
								...
								>
									...
									>
										...
										>
											<div 
												className="absolute top-0 left-0 w-[200%] h-full bg-gradient-to-r from-transparent via-white/10 to-transparent transform -translate-x-[100%] group-hover:translate-x-[100%] transition-transform duration-1500 ease-out skew-x-[-20deg]"
											></div>
										</div>

										{/* Corner decorative sparkles */}
										<div className={`absolute top-3 left-3 opacity-0 transition-opacity duration-500 ${hoveredCard === index ? 'opacity-70' : ''}`}>
											<SparklesIcon 
												className="h-4 w-4" 
												style={{ 
													color: isDark 
										...
										<div className={`absolute bottom-3 right-3 opacity-0 transition-opacity duration-500 ${hoveredCard === index ? 'opacity-70' : ''}`}>
											<SparklesIcon 
												className="h-4 w-4" 
												style={{ 
													color: isDark 
										...
										<CardContent className="flex h-full flex-col items-center justify-between relative z-10">
											...
											<div className="relative w-full mb-5 overflow-hidden rounded-lg group-hover:shadow-lg transition-all duration-500" 
												style={{
													boxShadow: isDark
											...
											<div className={`text-center w-full ${isDark ? "text-[rgba(230,210,255,0.95)]" : "text-[rgba(50,50,70,0.95)]"}`}>
												<h3 className={`text-lg font-semibold mb-4 tracking-wide ${
													isDark ? "text-[rgba(240,225,255,1)]" : "text-[rgba(103,80,164,1)]"
												}`}
												style={{
													textShadow: isDark
																...
																<div className="overflow-y-auto flex-grow px-2 relative z-10">
																	<DialogDescription 
																		className={`text-justify ${
																			isDark ? "text-[rgba(230,210,255,0.95)]" : "text-[rgba(50,50,70,0.95)]"
																		}`}
																	>
																		<motion.div 
																			initial={{ opacity: 0, y: 20 }}
																			...
																			<div className="text-center md:text-left">
																				...
																				<span 
																					className={`text-2xl font-bold ${
																						isDark ? "text-[rgba(230,210,255,0.95)]" : "text-[rgba(103,80,164,0.9)]"
																					}`}
																				>
																					{ps.sdg_id}
																				</span>
																				<div className={`mt-2 inline-block rounded-md px-3 py-1 ${sdgColorMap[ps.sdg_id]}`}>
																					{sdgIcon(ps.sdg_id)}
																					{`SDG-${ps.sdg_id}`}
																				</div>
																			</div>
																		...
																		>
																			...
																			<div className="leading-relaxed">{ps.description}</div>
																		</motion.div>
