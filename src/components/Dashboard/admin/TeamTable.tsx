...
const columns: ColumnDef<TeamWithActions>[] = [
	...
	},
	{
		...
		cell: ({ row }) => (
			<div>
				{row.original.team_leader ? row.original.team_leader.college || 'Unknown' : 'No leader assigned'}
			</div>
		),
	},
	{
					...
					{isModalOpen && (
						<Modal
							...
							<ModalHeader>
								...
								{/* Accent line */}
								<div className="mx-4 h-[1px]" style={{
							...
							<ModalBody>
								<div className="flex w-full flex-col items-start justify-start gap-5 p-4">
									...
									<div className={`flex w-full items-stretch justify-center gap-3 rounded-xl p-2 ${
									...
									{/* Team members list */}
									<ScrollArea className={`mt-4 flex max-h-[12rem] w-full flex-col items-start justify-start rounded-lg ${
										isDark 
										...
										{row.original.members.map((member, index) => (
											<div
												...
												className={`flex w-[100%] items-center justify-start gap-5 p-4 ${
													...
													index === row.original.members.length - 1 ? "" : isDark ? "border-b border-slate-700/50" : "border-b border-slate-200/50"
												}`}>
												<BoringAvatar
													size={40}
												...
												<div className="flex flex-col items-start justify-start">
													<span className={`text-lg ${isDark ? 'text-white' : 'text-slate-800'}`}>
														{member.username}
													</span>
			...
			>
				...
				<div className="relative flex w-[100%] items-end justify-between mb-6">
					<div>
						...
						style={{fontFamily: "var(--font-playfair-display)",}}>Teams</p>
					</div>
					<Input
				...
				<div className="relative">
					...
					/>
					{loading && (
						<div className={`my-6 flex items-center justify-center rounded-xl p-4 ${
							isDark 
								? 'bg-slate-800/40 border border-slate-700/50' 
								: 'bg-white/40 border border-slate-200/50'
						} backdrop-blur-sm`}>
