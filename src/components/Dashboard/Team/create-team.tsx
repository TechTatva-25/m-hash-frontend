		...
		<Card
			...
			<CardHeader className="relative z-10">
				...
				<CardDescription className="text-[hsl(var(--foreground))]/80">
					Create a new team and invite your friends to join.
				</CardDescription>
				{selectedUsers.length > 0 && (
					<Badge
						className="absolute right-6 top-5 z-10 flex flex-row backdrop-blur-xl"
						variant="secondary"
						style={{
							background: isDark
			...
			<CardContent className="relative z-10 px-2 pb-2 sm:px-6 sm:pb-6">
				...
				/>
				<div
					className="mb-4 flex flex-col rounded-md border-0 p-2"
					style={{
						background: isDark
				...
				<div
					className="mb-4 flex min-h-16 flex-row flex-wrap items-center gap-2 rounded-md border-0 p-2"
					style={{
						background: isDark
							? "linear-gradient(to bottom right, rgba(255, 255, 255, 0.04), rgba(255, 255, 255, 0.02))"
							: "linear-gradient(to bottom right, rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.3))",
					...
					{selectedUsers.map((user) => (
						<div
							key={user._id}
							className="flex flex-row items-center rounded-md border-0 bg-transparent p-2">
				...
				{/* Glassmorphic Button inspired by Send Message button */}
				<div className="mt-2 relative group">
					<button
						onClick={handleCreateTeam}
						disabled={disabled}
						className="group relative w-full inline-flex items-center justify-center px-8 py-4 text-base font-medium text-white transition-all duration-500 ease-out hover:scale-105 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60">
						{" "}
						{/* Outer border ring */}
						<div
							className="absolute inset-0 rounded-full border-2 transition-all duration-300 group-hover:border-white/40"
							style={{
						...
						<div
							className="absolute inset-1 rounded-full shadow-2xl transition-all duration-300 group-hover:bg-white/20"
							style={{
								background: isDark
