			...
			{moment(stage.start_date).isBefore(new Date()) && moment(stage.end_date).isAfter(new Date()) ? (
				<Badge 
					...
					<div className="flex items-center space-x-2">
						...
						<span className="text-xs" style={{ color: `${isDark ? 'rgba(255, 220, 150, 0.9)' : 'rgba(180, 120, 0, 0.9)'}` }}>In Progress</span>
					</div>
				</Badge>
			) : null}
			{moment(stage.end_date).isBefore(new Date()) ? (
				<Badge 
					...
					style={{
						background: `${isDark ? 'rgba(30, 30, 40, 0.6)' : 'rgba(255, 255, 255, 0.6)'}`,
						backdropFilter: "blur(10px)",
						border: stage._id === currentStage?._id ? 
						...
						boxShadow: stage._id === currentStage?._id ? 
					...
					<div className="flex items-center space-x-2">
						{stage._id === currentStage?._id ? (
							<>
								...
								<span className="text-xs" style={{ color: `${isDark ? 'rgba(255, 200, 200, 0.9)' : 'rgba(180, 30, 30, 0.9)'}` }}>Not Qualified</span>
							</>
						) : elapsedNotAttemptedStages.includes(stage) ? (
							<>
								<span className="relative flex h-3 w-3 items-center justify-center">
								...
								<span className="text-xs" style={{ color: `${isDark ? 'rgba(200, 200, 200, 0.9)' : 'rgba(100, 100, 100, 0.9)'}` }}>Elapsed</span>
							</>
						) : (
							<>
								<span className="relative flex h-3 w-3 items-center justify-center">
								...
								<span className="text-xs" style={{ color: `${isDark ? 'rgba(200, 255, 200, 0.9)' : 'rgba(0, 150, 50, 0.9)'}` }}>Qualified</span>
							</>
						)}
					</div>
				</Badge>
			...
			{moment(stage.start_date).isAfter(new Date()) ? (
				<Badge 
					...
					<div className="flex items-center space-x-2">
						...
						<span className="text-xs" style={{ color: `${isDark ? 'rgba(180, 200, 255, 0.9)' : 'rgba(50, 100, 200, 0.9)'}` }}>Upcoming</span>
					</div>
				</Badge>
			) : null}
		</div>
...
const timelineDotVariants = cva(
	"relative row-start-1 row-end-1 flex size-6 items-center justify-center rounded-full border border-current",
	{
		variants: {
			status: {
				default: "",
	...
	({ className, status, CustomIcon, bg_color, side, ...props }, ref) => {
		...
		<motion.div
			...
			<div
				...
				{...props}>
				<div 
					className={cn("absolute h-7 w-7 rounded-full")} 
					style={{
						background: bg_color,
						boxShadow: `0 0 20px ${isDark ? 'rgba(103, 80, 164, 0.5)' : 'rgba(132, 95, 220, 0.3)'}`,
						opacity: 0.9
					}}
				/>
				<CustomIcon 
					className="absolute z-10 h-4 w-4 text-white" 
					style={{
						filter: `drop-shadow(0 0 3px ${isDark ? 'rgba(255, 255, 255, 0.5)' : 'rgba(103, 80, 164, 0.5)'})`
					}}
				/>
			</div>
		</motion.div>
...
const TimelineContent = React.forwardRef<HTMLDivElement, TimelineContentProps>(({ className, side, ...props }, ref) => {
	...
	return (
		<motion.div
			...
			<div 
				className={cn(timelineContentVariants({ side }), "cursor-pointer transition-all hover:scale-[1.02]", className)} 
				ref={ref} 
				style={{
					transition: "transform 0.3s ease",
					boxShadow: `0 0 30px ${isDark ? 'rgba(103, 80, 164, 0.1)' : 'rgba(132, 95, 220, 0.05)'}`
				}}
				{...props} 
			/>
		</motion.div>
