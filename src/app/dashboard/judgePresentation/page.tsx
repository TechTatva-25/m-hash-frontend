...
const Judge = (): React.JSX.Element => {
	...
	const filteredTeams = useMemo(() => {
		return teams
			.filter((team) => team.showInPresentation ?? false)
			.filter(
				(team) =>
	...
	return (
		<ContentLayout title="Presentation Round Scoreboard">
			<div className="w-[100%] py-5">
				<Breadcrumb>
					<BreadcrumbList className="text-[15px]">
						...
						<BreadcrumbSeparator slash />
						<BreadcrumbItem>
							<BreadcrumbPage className="font-semibold">Presentation Round Scoreboard</BreadcrumbPage>
						</BreadcrumbItem>
					</BreadcrumbList>
				</Breadcrumb>
			</div>
						...
						<Table>
							<TableHeader>
								<TableRow className="hover:bg-transparent">
									...
									{rounds
										.filter((round) => round.visibleInPresentation)
										.map((round) => (
											<TableHead
												key={round._id}
